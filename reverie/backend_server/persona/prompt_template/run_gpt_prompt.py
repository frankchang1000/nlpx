...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "gpt-5-nano", "max_tokens": 5, 
...
def run_gpt_prompt_daily_plan(persona, 
  ...
  gpt_param = {"engine": "gpt-5-nano", "max_tokens": 500, 
               "temperature": 1, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v2/daily_planning_v6_gpt5.txt"
...
def run_gpt_prompt_generate_hourly_schedule(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    # If the response contains embedded schedule entries, extract just the activity
    if 'Activity:' in cr:
      # Find the last Activity: part and extract what comes after it
      activity_match = re.search(r'Activity:\s*(.+?)(?:\n|$)', cr)
      if activity_match:
        cr = activity_match.group(1).strip()
    
    # Remove duplicate name patterns (e.g., "Isabella is Isabella is doing X")
    cr = re.sub(r'\b(\w+) is \1 is\b', r'\1 is', cr)
    ...
    # Ensure it doesn't start with the person's name again (generic for any name)
    name_pattern = r'^(\w+) is '
    match = re.match(name_pattern, cr, re.IGNORECASE)
    if match:
      cr = cr[len(match.group(0)):]
  ...
  def get_fail_safe(): 
    # More realistic fail-safe activities for a cafe owner
    import random
    activities = [
      "preparing for the day at the cafe",
      "working at Hobbs Cafe", 
      "serving customers at the cafe",
      "taking a break",
      "managing cafe operations",
      "cleaning and organizing the cafe"
    ]
    fs = random.choice(activities)
    return fs
  ...
  # gpt_param = {"engine": "gpt-5-nano", "max_tokens": 15, 
  ...
  gpt_param = {"engine": "gpt-5-nano", "max_tokens": 50, 
               "temperature": 0.5, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
  prompt_template = "persona/prompt_template/v2/generate_hourly_schedule_v2_gpt5.txt"
  prompt_input = create_prompt_input(persona, 
                                     curr_hour_str, 
                                     p_f_ds_hourly_org,
                                     hour_str, 
...
def run_gpt_prompt_task_decomp(persona, 
  ...
  print ("?????")
  print (prompt)
  # Use ChatGPT_request instead of the deprecated GPT_request
  from .gpt_structure import ChatGPT_request
  try:
    raw_output = ChatGPT_request(prompt)
    # Check if we got a valid response
    if raw_output and raw_output not in ["TOKEN LIMIT EXCEEDED", "ChatGPT ERROR"]:
      output = __func_clean_up(raw_output, prompt=prompt)
    else:
      print(f"Got invalid response: {raw_output}")
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False): 
  ...
  def get_fail_safe(): 
    # Provide varied emoji fail-safes instead of always the same one
    import random
    emojis = ["😊", "🤔", "😋", "🎯", "✨", "🏃", "💭", "🎨", "📚", "☕", "🌟", "💪", "🎵"]
    fs = random.choice(emojis)
    return fs
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False): 
  ...
  gpt_param = {"engine": "gpt-5-nano", "max_tokens": 30, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
...
def run_gpt_prompt_new_decomp_schedule(persona, 
  ...
  try:
    ...
    if raw_output and raw_output not in ["TOKEN LIMIT EXCEEDED", "ChatGPT ERROR"]:
      ...
      print(f"Got invalid response in new_decomp_schedule: {raw_output}")
      output = fail_safe
  except Exception as e:
    print(f"Error in new_decomp_schedule: {e}")
    output = fail_safe
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved,test_input=None, 
  ...
  gpt_param = {"engine": "gpt-5-nano", "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc,
  ...
  try:
    ...
    if raw_output and raw_output not in ["TOKEN LIMIT EXCEEDED", "ChatGPT ERROR"]:
      ...
      print(f"Got invalid response in create_conversation: {raw_output}")
      output = fail_safe
  except Exception as e:
...
def run_gpt_prompt_keyword_to_thoughts(persona, keyword, concept_summary, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "gpt-5-nano", "max_tokens": 40, 
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False): 
  ...
  # gpt_param = {"engine": "gpt-5-nano", "max_tokens": 3, 
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "gpt-5-nano", "max_tokens": 150, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_agent_chat(maze, persona, target_persona,
  ...
  # gpt_param = {"engine": "gpt-5-nano", "max_tokens": 2000, 
  #              "temperature": 0.7, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "gpt-5-nano", "max_tokens": 250, 
               "temperature": 1, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
